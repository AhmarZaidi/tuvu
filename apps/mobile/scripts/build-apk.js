...
const selectedArch = archArg ? archArg.split('=')[1] : (isAllArch ? 'armeabi-v7a,arm64-v8a,x86,x86_64' : 'arm64-v8a');
...
async function promptVersion(currentVersion) {
  ...
  return new Promise(resolve => {
    rl.question(`App version [${currentVersion}] (press Enter to keep, or type new version): `, answer => {
      rl.close();
  ...
  if (fs.existsSync(appJsonPath)) {
    ...
    if (appJson.expo) {
      ...
      if (changed) {
        ...
        console.log(`✓ Updated app.json version -> ${newVersion} (versionCode: ${appJson.expo.android?.versionCode})`);
      }
    }
  }
...
function ensureAndroidDir() {
  if (!fs.existsSync(androidDir)) {
    console.log('Generating android project via expo prebuild...');
    const npxCmd = process.platform === 'win32' ? 'npx.cmd' : 'npx';
    const res = spawnSync(npxCmd, ['expo', 'prebuild', '--platform', 'android', '--no-install'], {
      cwd: projectRoot,
      stdio: 'inherit',
...
function ensureSdkProperties() {
  ...
  if (sdkPath) {
    ...
    console.log(`Auto-configured Android SDK in android/local.properties: ${formattedSdkDir}`);
  } else {
  ...
  if (fs.existsSync(gradlePropsPath)) {
    ...
    if (bestJdk) {
      ...
      if (content.includes('org.gradle.java.home=')) {
        content = content.replace(/org\.gradle\.java\.home=.*/, `org.gradle.java.home=${formattedJdk}`);
      } else {
    ...
    if (content.includes('org.gradle.jvmargs=-Xmx2048m')) {
      content = content.replace('org.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=512m', 'org.gradle.jvmargs=-Xmx4096m -XX:MaxMetaspaceSize=1024m');
      changed = true;
    }
...
async function main() {
  ...
  if (bestJdk) {
    ...
    buildEnv[pathKey] = `${path.join(bestJdk, 'bin')}${path.delimiter}${currentPath}`;
  ...
  const namedApkPath = path.join(buildsDir, `tuvu-v${selectedVersion}-${variant}.apk`);
