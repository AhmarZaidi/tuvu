...
export async function openLibrarySearch(env: Env, query: string, limit: number) {
  ...
  const data = await cachedJson<{ docs?: unknown[] }>(env, "openlibrary", `search:${query.toLowerCase()}`, providerTtls.openLibrarySearch, () => fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=${limit}`, openLibraryHeaders(email ?? undefined)));
...
export async function openLibrarySubject(env: Env, subject: string, limit: number) {
  const email = await providerCredential(env, { provider: "openlibrary", key: "OPEN_LIBRARY_CONTACT_EMAIL" });
  const data = await cachedJson<{ works?: unknown[] }>(env, "openlibrary", `subject:${subject.toLowerCase()}`, providerTtls.openLibrarySubject, () => fetch(`https://openlibrary.org/subjects/${encodeURIComponent(subject)}.json?limit=${limit}`, openLibraryHeaders(email ?? undefined)));
...
export async function openLibraryFetchDetails(env: Env, providerId: string) {
  ...
  const [workData, editionsData] = await Promise.all([
    cachedJson<any>(env, "openlibrary", `work:${providerId}`, providerTtls.openLibraryDetail, () => fetch(`https://openlibrary.org/works/${providerId}.json`, headers)),
    cachedJson<any>(env, "openlibrary", `editions:${providerId}`, providerTtls.openLibraryDetail, () => fetch(`https://openlibrary.org/works/${providerId}/editions.json`, headers)),
  ]);
...
function normalizeOpenLibrarySubject(item: unknown): ProviderResult | null {
  ...
  return { provider: "openlibrary", providerId: key.replace(/^\/works\//, ""), type: "book", title, overview: Array.isArray(record.authors) ? `By ${record.authors.slice(0, 3).map((a: any) => a.name).join(", ")}` : null, posterPath: coverId ? `https://covers.openlibrary.org/b/id/${coverId}-M.jpg` : null, backdropPath: null, releaseDate: firstPublishYear ? `${firstPublishYear}-01-01` : null, year: firstPublishYear, sourceUrl: `https://openlibrary.org${key}`, rating: null, popularity: numberValue(record.edition_count), attribution: providerAttributions.openlibrary, extendedDataJson: null };
}
